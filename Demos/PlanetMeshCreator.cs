...
{
    public static Mesh[] CreateThickPlanetMesh(float thickness, int numberOfLayers, float radius, int subdivisionSteps, Vector3 scaling, BufferPool pool, IThreadDispatcher dispatcher = null)
    {
        var meshes = new Mesh[numberOfLayers];

        if (numberOfLayers == 1)
        {
            meshes[0] = CreatePlanetMesh(radius, subdivisionSteps, scaling, pool, dispatcher);
        }
        else
        {
            ...
            {
                float currentRadius = radius - (thickness * 0.5f) + (i * layerSpacing);
                meshes[i] = CreatePlanetMesh(currentRadius, subdivisionSteps, scaling, pool, dispatcher);
            }
        }
